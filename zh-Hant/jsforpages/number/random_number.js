...
function random_number() {
    ...
    // }
    if (max == "") {
      document.getElementById("result").value += "您尚未輸入最大值\n";
    }
    if (min == "") {
      document.getElementById("result").value += "您尚未輸入最小值\n";
    }
    ...
    // }
      ...
      if (min >= max) {
        document.getElementById("result").value += "最小數必須小於最大數\n";
      }
      if (choose_num == "") {
        document.getElementById("result").value += "您尚未輸入範圍\n";
      }
    if (choose_num > (max-min+1) || choose_num <= 0) {
      document.getElementById("result").value ="所選取的數字超過範圍 或是 最小數大於等於最大數";
    }
    else {
